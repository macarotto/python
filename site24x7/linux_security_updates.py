        ...
        try:
            ...
            for line in fileContent:
                ...
                if key == 'ID':
                    ...
                    id_like_val = value.lower()

            self.os_name = id_like_val or id_val
            if not self.os_name:
                self.maindata['msg'] = "Distro information not found in {}".format(distro_file)
                self.maindata['status'] = 0
                return self.maindata

            if ("debian" in self.os_name) or ("ubuntu" in self.os_name):
            ...
            elif ("fedora" in self.os_name) or ("centos" in self.os_name) or ("rhel" in self.os_name):
            ...
            elif ("suse" in self.os_name) or ("opensuse" in self.os_name) or ("sles" in self.os_name):
